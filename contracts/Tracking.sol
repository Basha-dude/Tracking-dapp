   ...
   event ShipmentCreated(
    ...
    constructor() {
     ...
     function createShipment(address _receiver,uint256 _pickupTime,uint256 _distance,uint256 _price
      ...
      require(msg.value == _price);
       ...
       emit ShipmentCreated(msg.sender, _receiver, _pickupTime, _distance, _price );
        ...
        function startShipment( address _sender,address _receiver, uint256 index) public {
          ...
          require(shipment.status ==   ShipmentStatus.PENDING);
      ...
      function completeShipment(address _sender,address _receiver, uint256 index) public {
         Shipment storage shipment = Shipments[_sender][index];
          TypeShipment storage typeShipment = typeShipments[index];
          require(shipment.receiver == _receiver);
          require(shipment.status ==   ShipmentStatus.IN_TRANSIT);
          require(!shipment.isPaid); 
        ...
        emit ShipmentDeliverd(_sender, _receiver, amount);
        emit ShipmentPaid(_sender, _receiver, shipment.price);
      }

   function getShipment(address _sender, uint256 _index) public view returns ( address, address, uint256, uint256,
